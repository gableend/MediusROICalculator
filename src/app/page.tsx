...
// Processing cycle time benchmarks (days)
const BENCHMARKS = {
  po: {
    average: 6.7,
    best: 2.6,
...
// NOTE: Updated to align with the benchmark story used in the UI
// (68.9% average → 96.3% best-in-class)
...
// Percent position helper for slider markers
function positionPct(value: number, min: number, max: number): string {
...
function BenchmarkChart({
  ...
  return (
    <div
      ref={ref}
      className="bg-white rounded-lg p-4 md:p-6 h-full flex flex-col transition-all duration-300 hover:shadow-lg"
    >
      ...
      <div className="flex-1 flex items-end justify-center gap-6 md:gap-10 min-h-[120px] md:min-h-[140px] pb-2">
        <div className="flex flex-col items-center h-full justify-end">
          <span
            className={`text-xs md:text-sm font-bold text-[#2d4242] mb-2 transition-opacity duration-500 ${
        ...
        <div className="flex flex-col items-center h-full justify-end">
          <span
            className={`text-xs md:text-sm font-bold text-primary mb-2 transition-opacity duration-500 ${
              isInView ? "opacity-100" : "opacity-0"
            }`}
          >
            {bestLabel}
...
export default function Home() {
  ...
  const [poInvoiceVolume, setPoInvoiceVolume] = useState(
    DEFAULT_VALUES.poInvoiceVolume
  );
  const [nonPoInvoiceVolume, setNonPoInvoiceVolume] = useState(
    DEFAULT_VALUES.nonPoInvoiceVolume
  );
  const [currentDaysToProcess, setCurrentDaysToProcess] = useState(
    DEFAULT_VALUES.currentDaysToProcess
  );
  const [poManualInterventionPct, setPoManualInterventionPct] = useState(
    DEFAULT_VALUES.poManualInterventionPct
  );
  const [annualPayablesSpend, setAnnualPayablesSpend] = useState(
    DEFAULT_VALUES.annualPayablesSpend
  );
  ...
  const nonPoDaysSavedAvg = Math.max(
    0,
    currentDaysToProcess - BENCHMARKS.nonPo.average
  );
  const nonPoDaysSavedBest = Math.max(
    0,
    currentDaysToProcess - BENCHMARKS.nonPo.best
  );

  // Touchless calculations (PO only)
  ...
  // Incremental improvement vs current state
  const touchlessAdditionalAvg = Math.max(
    0,
    poInvoiceVolume * (TOUCHLESS_RATE.average - currentTouchlessRate)
  );
  const touchlessAdditionalBest = Math.max(
    0,
    poInvoiceVolume * (TOUCHLESS_RATE.best - currentTouchlessRate)
  );
  ...
  const handleEmailSubmit = useCallback(
    (e: React.FormEvent) => {
      ...
      setTimeout(() => {
        // Replace with your real lead capture
        console.log("Lead captured:", {
          ...
          calculatorData: {
            ...
            poInvoiceVolume,
            nonPoInvoiceVolume,
            currentDaysToProcess,
            poManualInterventionPct,
            annualPayablesSpend,
            paymentSavingsLow,
            paymentSavingsHigh,
          },
  ...
  useEffect(() => {
    ...
    if (po)
      setPoInvoiceVolume(
        Math.min(500000, Math.max(0, parseInt(po, 10) || DEFAULT_VALUES.poInvoiceVolume))
      );
    if (npo)
      setNonPoInvoiceVolume(
        Math.min(500000, Math.max(0, parseInt(npo, 10) || DEFAULT_VALUES.nonPoInvoiceVolume))
      );
    if (days)
      setCurrentDaysToProcess(
        Math.min(20, Math.max(1, parseFloat(days) || DEFAULT_VALUES.currentDaysToProcess))
      );
    if (manual)
      setPoManualInterventionPct(
        Math.min(80, Math.max(0, parseInt(manual, 10) || DEFAULT_VALUES.poManualInterventionPct))
      );
    if (spend)
      setAnnualPayablesSpend(
        Math.min(
          2000000000,
          Math.max(5000000, parseInt(spend, 10) || DEFAULT_VALUES.annualPayablesSpend)
        )
      );
  ...
  // Slider ranges (keep in one place)
  const DAYS_MIN = 1;
  const DAYS_MAX = 20;
  const MANUAL_MIN = 0;
  const MANUAL_MAX = 80;
    ...
    <>
      ...
      >
        <div className="relative container mx-auto px-4 text-center">
          ...
          >
            See My Savings
            <svg
              className="w-4 h-4 transition-transform duration-300 group-hover:translate-y-1 group-hover:animate-bounce"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
      ...
      <section className="relative bg-[#f4f4f2] pt-16 md:pt-20 pb-28 md:pb-36">
        <div
          ...
          style={{ transform: "translateY(-99%) scaleX(-1)" }}
        >
          <svg
          ...
          >
            <path
              d="M-0.00012207 160V26.2023C118.348 14.5718 260.76 8.19165 429.194 2.59521C823.48 -10.5054 1218.72 25.6423 1512 118.946L1512 160L-0.00012207 160Z"
              fill="#f4f4f2"
            />
          </svg>
        </div>
              ...
              <div className="grid grid-cols-2 gap-4 mt-4">
                <div className="col-span-2 grid grid-cols-3 gap-2 mb-2">
                  <div className="bg-white rounded-lg p-3 text-center transition-all duration-200 hover:shadow-lg hover:-translate-y-1">
                    ...
                    <p className="text-[9px] md:text-[10px] text-[#2d4242]/70 uppercase tracking-wide">
                      PO Touchless Capture Rate
                    </p>
                  </div>
                  <div className="bg-white rounded-lg p-3 text-center transition-all duration-200 hover:shadow-lg hover:-translate-y-1">
                ...
                <BenchmarkChart
                  title="Touchless processing rate"
                  averageValue={68.9}
                  bestValue={96.3}
                  averageLabel="68.9%"
                  bestLabel="96.3%"
                ...
                />
                <BenchmarkChart
                  title="Total invoice processing cycle time"
                  averageValue={5.1}
                  bestValue={1.4}
                  averageLabel="5.1 days"
                  bestLabel="1.4 days"
                  maxValue={6}
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 mt-4">
                <div className="col-span-2 grid grid-cols-3 gap-2 mb-2">
                  <div className="bg-white rounded-lg p-3 text-center transition-all duration-200 hover:shadow-lg hover:-translate-y-1">
                    ...
                    <p className="text-[9px] md:text-[10px] text-[#2d4242]/70 uppercase tracking-wide">
                      Non-PO Touchless Capture Rate
                    </p>
                  </div>
                  <div className="bg-white rounded-lg p-3 text-center transition-all duration-200 hover:shadow-lg hover:-translate-y-1">
                    <div className="text-lg md:text-xl font-bold text-primary">65.7% → 99.5%</div>
                    <p className="text-[9px] md:text-[10px] text-[#2d4242]/70 uppercase tracking-wide">
                      Non-PO Automatic Routing Rate
                    </p>
                  </div>
                  <div className="bg-white rounded-lg p-3 text-center transition-all duration-200 hover:shadow-lg hover:-translate-y-1">
                    <div className="text-lg md:text-xl font-bold text-primary">
                      {formatDays(3.3)} {dayLabel(3.3)} → {formatDays(1.0)} {dayLabel(1.0)}
                    </div>
                    <p className="text-[9px] md:text-[10px] text-[#2d4242]/70 uppercase tracking-wide">
                      Non-PO Average Approval Time
                    </p>
                  </div>
                </div>

                <BenchmarkChart
                  title="Non-PO Touchless Capture Rate"
                  averageValue={76.6}
                  bestValue={100}
                  averageLabel="76.6%"
                  bestLabel="100%"
              ...
              <div className="relative flex flex-col lg:flex-row items-center justify-between gap-6">
                <div className="flex items-center justify-center lg:justify-start flex-shrink-0">
                  <div className="inline-flex items-center gap-2 px-4 py-2.5 bg-primary/10 rounded-full border border-primary/20 animate-pulse">
                    ...
                    <span className="text-xs md:text-sm font-semibold text-primary uppercase tracking-wide whitespace-nowrap">
                      ROI Benchmark
                    </span>
                  </div>
                </div>
      ...
      <section id="calculator-section" className="relative bg-[#2d4242] py-16 md:py-20 pb-28 md:pb-32">
              ...
              <div className="space-y-8">
                ...
                <div className="space-y-3">
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Non-PO invoices account for{" "}
                    {Math.round((nonPoInvoiceVolume / (poInvoiceVolume + nonPoInvoiceVolume || 1)) * 100)}% of total invoice volume.
                  </p>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="bg-gray-50 rounded-lg px-4 py-3 flex items-center justify-between border border-gray-200">
                    <span className="font-mono text-lg font-semibold text-[#2d4242]">{formatDays(currentDaysToProcess)}</span>
                    <span className="text-sm text-gray-500">days</span>
                  </div>

                  <div className="relative">
                    <Slider value={[currentDaysToProcess]} onValueChange={handleDaysChange} min={DAYS_MIN} max={DAYS_MAX} step={0.1} className="w-full" />
                    <div className="absolute top-1/2 left-0 right-0 h-0 pointer-events-none" style={{ transform: "translateY(-50%)" }}>
                      <span
                        ...
                        style={{ left: positionPct(BENCHMARKS.nonPo.best, DAYS_MIN, DAYS_MAX), transform: "translateX(-50%)" }}
                      />
                      <span
                        className="absolute w-1.5 h-1.5 bg-gray-400 rounded-full"
                        style={{ left: positionPct(BENCHMARKS.po.best, DAYS_MIN, DAYS_MAX), transform: "translateX(-50%)" }}
                      />
                      <span
                        className="absolute w-1.5 h-1.5 bg-gray-400 rounded-full"
                        style={{ left: positionPct(BENCHMARKS.nonPo.average, DAYS_MIN, DAYS_MAX), transform: "translateX(-50%)" }}
                      />
                      <span
                        className="absolute w-1.5 h-1.5 bg-gray-400 rounded-full"
                        style={{ left: positionPct(BENCHMARKS.po.average, DAYS_MIN, DAYS_MAX), transform: "translateX(-50%)" }}
                      />
                    </div>
                  </div>
                  ...
                  <p className="text-xs text-gray-500">
                    Benchmarks: Best NPO ({BENCHMARKS.nonPo.best.toFixed(1)}) • Best PO ({BENCHMARKS.po.best.toFixed(1)}) • Avg NPO ({BENCHMARKS.nonPo.average.toFixed(1)}) • Avg PO ({BENCHMARKS.po.average.toFixed(1)})
                  </p>
                </div>
                ...
                <div className="space-y-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">
                        Reducing manual intervention increases touchless, fully automated invoice processing.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-50 rounded-lg px-4 py-3 flex items-center justify-between gap-3 border border-gray-200">
                    ...
                    <span className="text-sm text-gray-500 flex items-center justify-end flex-wrap text-right leading-tight">
                      <span className="mx-2 text-gray-400">·</span>
                      <span>Current touchless rate: {100 - poManualInterventionPct}%</span>
                    </span>
                  </div>

                  <div className="relative">
                    <Slider value={[poManualInterventionPct]} onValueChange={handleManualInterventionChange} min={MANUAL_MIN} max={MANUAL_MAX} step={1} className="w-full" />
                    <div className="absolute top-1/2 left-0 right-0 h-0 pointer-events-none" style={{ transform: "translateY(-50%)" }}>
                      <span
                        className="absolute w-1.5 h-1.5 bg-gray-400 rounded-full"
                        style={{ left: positionPct(6, MANUAL_MIN, MANUAL_MAX), transform: "translateX(-50%)" }}
                      />
                      <span
                        className="absolute w-1.5 h-1.5 bg-gray-400 rounded-full"
                        style={{ left: positionPct(33, MANUAL_MIN, MANUAL_MAX), transform: "translateX(-50%)" }}
                      />
                    </div>
                  </div>
                  ...
                  <p className="text-xs text-gray-500">Benchmarks: Best (6%) • Avg (33%)</p>
                </div>
            ...
            <div className="bg-white rounded-lg p-6 md:p-8 lg:p-10">
              ...
              <div className="space-y-6">
                ...
                <div className="p-5 bg-gray-50 rounded-lg border border-gray-200 transition-all duration-200 hover:shadow-md">
                  ...
                  <div className="text-2xl md:text-3xl font-bold text-primary">
                    {poDaysSavedAvg > 0 || poDaysSavedBest > 0 ? (
                      <AnimatedValue>
                        {formatDaysRange(poDaysSavedAvg, poDaysSavedBest)} {dayLabel(Math.floor(poDaysSavedBest))}
                      </AnimatedValue>
                    ) : (
                ...
                <div className="p-5 bg-gray-50 rounded-lg border border-gray-200 transition-all duration-200 hover:shadow-md">
                  ...
                  <div className="text-2xl md:text-3xl font-bold text-primary">
                    {nonPoDaysSavedAvg > 0 || nonPoDaysSavedBest > 0 ? (
                      <AnimatedValue>
                        {formatDaysRange(nonPoDaysSavedAvg, nonPoDaysSavedBest)} {dayLabel(Math.floor(nonPoDaysSavedBest))}
                      </AnimatedValue>
                    ) : (
                      <span className="text-[#2d4242]/50">Already optimized</span>
                    )}
                ...
                <div className="p-5 bg-gray-50 rounded-lg border border-gray-200 transition-all duration-200 hover:shadow-md">
                  ...
                  <div className="text-2xl md:text-3xl font-bold text-primary mb-2">
                    {touchlessAdditionalAvg > 0 || touchlessAdditionalBest > 0 ? (
                      <AnimatedValue>
                        +{formatNumber(touchlessAdditionalAvg)} – +{formatNumber(touchlessAdditionalBest)}
                      </AnimatedValue>
                    ) : (
                  ...
                  <p className="text-sm text-[#2d4242]/70">
                    Total touchless:{" "}
                    <AnimatedValue>
                      {formatNumber(touchlessTotalAvg)} – {formatNumber(touchlessTotalBest)}
                    </AnimatedValue>{" "}
                    invoices
                  </p>
                </div>
                ...
                <div className="p-6 bg-primary/5 rounded-lg border-2 border-primary/20 transition-all duration-200 hover:shadow-md">
                  ...
                  <div className="text-2xl md:text-3xl font-bold text-primary">
                    <AnimatedValue>
                      {formatCurrency(paymentSavingsLow, currencySymbol)} – {formatCurrency(paymentSavingsHigh, currencySymbol)}
                    </AnimatedValue>
                  </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 w-full overflow-hidden leading-none">
          <svg className="w-full h-12 md:h-16 lg:h-20" viewBox="0 0 1512 115" preserveAspectRatio="none" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
              d="M1512 115H0V45C0 45 145 0 378 0C611 0 780 99 1165 99C1403 99 1512 20 1512 20V115Z"
              fill="white"
              transform="scale(-1,1) translate(-1512, 0)"
            />
          </svg>
        </div>
      </section>
      ...
      <section className="bg-[#f4f4f2] py-20 md:py-24">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold text-[#2d4242] mb-8">
            Ready to transform your spend management with Medius?
          </h2>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <button
              type="button"
              onClick={() => setIsEmailModalOpen(true)}
              className="inline-flex items-center justify-center px-8 py-4 bg-primary text-white font-semibold rounded hover:bg-primary/90 transition-colors uppercase tracking-wide text-sm"
            >
              Book a Demo
            </button>
            <button
              type="button"
              className="inline-flex items-center justify-center px-8 py-4 bg-[#1a1a1a] text-white font-semibold rounded hover:bg-gray-800 transition-colors uppercase tracking-wide text-sm"
            >
              Contact Us
            </button>
          </div>
        ...
        <div className="container mx-auto px-4 text-center">
          <div className="flex justify-center mb-4">
            <svg className="h-8 text-white" viewBox="0 0 120 24" fill="currentColor">
              <text x="0" y="18" fontFamily="Poppins, sans-serif" fontSize="18" fontWeight="600">
                medius
              </text>
            </svg>
          </div>
          <p className="text-white/60 text-sm">
            The estimated savings reflect the time saved across the entire purchase invoice handling process as a result of Medius automation&apos;s impact.
          </p>
        </div>
      </footer>
    </>
